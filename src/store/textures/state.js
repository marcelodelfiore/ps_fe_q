export default function () {
  return {
    list: [
      { id: "1", description: "Moído, disco #6" },
      { id: "2", description: "Moído, disco #10" },
      { id: "3", description: "Emulsificado" },
      { id: "4", description: "-" },
      { id: "5", description: "Em pó (para condimentos)" },
      { id: "6", description: "Em grãos (para condimentos)" },
      { id: "7", description: "Moída (para condimentos)" },
      { id: "8", description: "Moído, disco #4" },
      { id: "9", description: "Moído, disco #5" },
      { id: "10", description: "Moído, disco #8" },
      { id: "11", description: "Picado, em cubos de 1 cm" },
      { id: "12", description: "Picado, em cubos de 3 cm" }
    ]
  }
}
